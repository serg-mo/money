    ...
    watch: {
      ...
      subcategory: (val) => {
        ...
        stack.data.datasets.forEach((dataset, i) => {
          ...
          dataset.hidden = app.subcategory
            ? dataset.label != app.subcategory
            : false;
        });
        stack.update();
      },
...
function set_transactions(transactions) {
  ...
  app.transactions = jmespath.search(
    app.transactions,
    "[?category != 'Income']",
  ); // spending only
...
function section_one_setup() {
  ...
  stack.options.tooltips.itemSort = (a, b) => {
    return b.yLabel - a.yLabel;
  };
  stack.options.tooltips.filter = (v) => {
    return v.yLabel > 0;
  };
}
...
function key_handler(event) {
  ...
  if (keys.includes(key)) {
    ...
    if (key == "ArrowUp" || key == "ArrowDown") {
      ...
      let next = Math.max(
        Math.min(current + delta, app.TIME_RESOLUTIONS.length - 1),
        0,
      );
...
function make_comparisons(datetime) {
  ...
  */
  ...
  for (var i in comparisons) {
    ...
    comparisons[i] = filter_transactions(query).reduce(
      single_reducer("category"),
      {},
    );
  }
